 ...
 *
 * Free (no unlock): OpenRouter free-tier only (:free slug, $0 pricing, openrouter/free).
 * Paid (password): Venice, OpenRouter paid, Cerebras, Groq, NVIDIA — all burn credits.
 */
...
export function inferFree(providerId, model = {}) {
  ...
  // Venice is never free — every call burns Venice credits.
  ...
  // Cerebras / Groq / NVIDIA also burn credits → paid gate.
  return false;
}
...
export function inferCategories(providerId, model = {}) {
  ...
  // "Mainstream" = the flagship-tier release of a big-name family (GPT, Claude,
  // Grok, Gemini, Llama, DeepSeek, Qwen3, Mistral Large, Kimi, GLM, Nemotron
  // super/70b+) — not the latest bleeding-edge drop, just a solid known name.
  // Skip anything already flagged uncensored/roleplay so that lane stays separate.
  if (
    !/uncensored|dolphin|heretic|abliterated|mytho|role.?play|hermes/.test(hay) &&
    /claude|(^|\W)gpt-[3-9]|chatgpt|\bgrok\b|gemini|llama-3\.3|llama-4|llama-3\.1-(70b|405b)|deepseek|qwen3(?![\s-]*coder)|mistral-large|mixtral|kimi-k2|glm-[45]|nemotron.*(49b|70b|120b|253b|super)/.test(
      hay,
    )
  ) {
    cats.add('mainstream');

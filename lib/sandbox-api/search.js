...
/** Filename search may include larger sources (e.g. public/app.js). */
const MAX_SEARCH_FILE_BYTES = 400_000;
/** Snippet extraction still reads only the head — enough for keyword hits. */
const MAX_SNIPPET_READ_BYTES = 120_000;
...
async function extractSnippets(sandbox, relPath, keywords) {
  try {
    const cat = await sandbox.runCommand({
      cmd: 'bash',
      args: ['-c', `head -c ${MAX_SNIPPET_READ_BYTES} "${REPO_DIR}/${relPath}" 2>/dev/null`],
    });

...
const INCOMPLETE_PATCH_RE =
  /(?:^|\n)\s*(?:\/\/|#|\/\*|<!--)\s*\.\.\.\s*existing\b|(?:^|\n)\s*\/\/\s*Then in (?:the )?handler\b|\b\.\.\.\s*existing (?:code|imports|content|implementation|logic|handlers?)\b|\b(rest of (?:the )?file|unchanged below|keep the rest|same as before)\b|\bYOUR[_ -]?CODE[_ -]?HERE\b|\bINSERT[_ -]?CODE[_ -]?HERE\b|<placeholders?>/i;

function rejectIncomplete(relPath, content) {
  const text = typeof content === 'string' ? content : '';
  if (!text.trim()) return 'SANDBOX PROTECTED: empty file content';
  if (INCOMPLETE_PATCH_RE.test(text)) {
    return 'SANDBOX PROTECTED: incomplete patch / stub — write the FULL file (no “… existing …”)';
  }
  const rel = String(relPath || '').replace(/\\/g, '/');
  if (/^api\/.+\.js$/.test(rel) || /(^|\/)api\/.+\.js$/.test(rel)) {
    if (!/\bexport\s+default\b/.test(text)) {
      return 'SANDBOX PROTECTED: api handlers must include export default';
    }
    if (text.length < 200) {
      return 'SANDBOX PROTECTED: api file looks truncated — refusing write';
    }
  }
  if (/(^|\/)lib\/sandbox-api\/.+\.js$/.test(rel) || /(^|\/)lib\/config\.js$/.test(rel)) {
    if (text.length < 120) {
      return 'SANDBOX PROTECTED: critical lib looks truncated — refusing write';
    }
  }
  return null;
}

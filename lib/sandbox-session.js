...
export async function ensurePythonStack(sandbox) {
  try {
    const probe = await sandbox.runCommand({
      cmd: 'bash',
      args: ['-lc', `test -f ${PYTHON_MARKER} && python3 -m pip --version >/dev/null 2>&1`],
    });
    ...
    const install = await sandbox.runCommand({
      ...
      args: ['-lc', [
        ...
        '    dnf install -y python3 python3-pip python3-devel gcc 2>&1 | tail -n 20',
        '  elif command -v yum >/dev/null 2>&1; then',
        '    yum install -y python3 python3-pip python3-devel gcc 2>&1 | tail -n 20',
        '  else',
        ...
        `if [ -x ${VENV_DIR}/bin/pip ]; then ${VENV_DIR}/bin/pip install --upgrade pip -q; fi`,
        `mkdir -p /vercel/sandbox && touch ${PYTHON_MARKER}`,
        'echo "PYTHON_OK $(python3 --version) pip=$(python3 -m pip --version)"',
        `echo "VENV ${VENV_DIR} $([ -x ${VENV_DIR}/bin/python ] && echo ready || echo missing)"`,
      ].join('\n')],
      sudo: true,
    });

 ...
 *   {
 *     kind: 'image' | 'video',
 *     provider: 'cloudflare' | 'venice',
 *     model?: string,
 *     prompt: string,
 *     negativePrompt?: string,
 *     size?: string,          // image: 1024x1024 | video: 832x480 / 480x832
 *     seconds?: number,       // video length
 *     imageBase64?: string,   // Venice Edit only (do NOT send on text-to-image)
 *     mimeType?: string
 *   }
 *
...
/** Optional Cloudflare fallback chain. */
async function generateImageWithFallbacks({ prompt, size, negativePrompt, preferredCfModel = 'flux-schnell' }) {
  ...
  if (veniceImageKey()) {
    try {
      return await generateVeniceImage({ prompt, model: 'z-image-turbo', size, negativePrompt });
    } catch (err) {
...
export default async function handler(req, res) {
  ...
  try {
    if (kind === 'image') {
      ...
      // Venice = uncensored path (safe_mode:false). CF keeps host filters.
      if (provider === 'venice') {

...
var Sandbox = class Sandbox {
	...
	*/
	get expiresAt() {
		...
		return this.sandbox.expiresAt !== void 0 ? new Date(this.sandbox.expiresAt) : void 0;
	}
	/**
	...
	* });
	*/
	static async fork(params) {
		"use step";
		const { sourceSandbox: sourceName, ...overrides } = params;
		const credentialFields = params;
		const sourceSandbox = await Sandbox.get({
			...
			fetch: params.fetch,
			signal: params.signal,
			...require_types.getPrivateParams(params),
			name: sourceName,
			resume: false
		});
		const sourcePorts = sourceSandbox.routes.filter((r) => r.port !== sourceSandbox.interactivePort).map((r) => r.port);
		const copied = {
			...sourceSandbox.vcpus !== void 0 && { resources: { vcpus: sourceSandbox.vcpus } },
			...sourceSandbox.timeout !== void 0 && { timeout: sourceSandbox.timeout },
			...sourceSandbox.networkPolicy !== void 0 && { networkPolicy: sourceSandbox.networkPolicy },
			...sourceSandbox.tags !== void 0 && { tags: sourceSandbox.tags },
			...sourcePorts.length > 0 && { ports: sourcePorts },
			persistent: sourceSandbox.persistent,
			...sourceSandbox.snapshotExpiration !== void 0 && { snapshotExpiration: sourceSandbox.snapshotExpiration },
			...sourceSandbox.keepLastSnapshots !== void 0 && { keepLastSnapshots: sourceSandbox.keepLastSnapshots }
		};
		const snapshotId = sourceSandbox.currentSnapshotId;
		if (snapshotId) return Sandbox.create({
			...copied,
			...overrides,
			source: {
				type: "snapshot",
		...
		return Sandbox.create({
			...copied,
			...sourceSandbox.runtime !== void 0 && { runtime: sourceSandbox.runtime },
			...overrides
		});
	}
	/**
	...
	*/
	async openInteractive(opts) {
		"use step";
		return this.withResume(() => this.session.openInteractive(opts), opts?.signal);
	}
	/**
	...
	*/
	async update(params, opts) {
		...
		if (params.timeout !== void 0 && this.session?.status === "running") {
			const increment = params.timeout - this.session.timeout;
			if (increment > 0) try {
				await this.session.extendTimeout(increment, opts);
			} catch (err) {
				if (!isSandboxStoppedError(err) && !isSandboxStoppingError(err)) throw err;
			}
		}
